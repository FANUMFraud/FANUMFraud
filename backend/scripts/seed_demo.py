...
DEMO_COMPANIES: list[DemoCompany] = [
    ...
    DemoCompany(
        ...
        minor_probability=0.09,
        risk_range=(32, 60),
        shock_category="money_laundering",
        shock_risk=66,
    ),
    DemoCompany(
...
def main() -> None:
    ...
    try:
        ...
        companies = [_upsert_company(db, item) for item in DEMO_COMPANIES]
        db.commit()
        ...
        for company, config in zip(companies, DEMO_COMPANIES, strict=True):
            signals: list[RiskSignal] = []
            score_points: list[float] = []
            start = _utc_now() - timedelta(days=args.days)

            for index in range(args.per_company):
                published_at = _published_at(start, args.days, args.per_company, index, rng)
                signal = _article_signal(config, index, args.per_company, rng)
...
def _article_signal(config: DemoCompany, index: int, per_company: int, rng: random.Random) -> ArticleSignal:
    shock_offsets = {per_company - 3}
    if config.shock_category and index in shock_offsets:
    ...
    roll = rng.random()
    if roll < config.risk_probability:
        return ArticleSignal(
    ...
    if roll < config.risk_probability + config.minor_probability:
...
def _published_at(start: datetime, days: int, per_company: int, index: int, rng: random.Random) -> datetime:
    ...
    if published_at > latest_allowed:
        return latest_allowed - timedelta(minutes=rng.randint(0, 180))
    return published_at
